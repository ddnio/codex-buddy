...
### Step 1：确定主题（failure-first）

**优先处理 `STATUS.md` 里 `confirmed_failures` 中标注 OPEN 的问题。**

如果 `health_status = HEALTHY` 且无 OPEN failures，再读 `CHANGELOG.md` 最新版本的"下轮 Agenda"，取第一个 `[ ]` 未完成项。
